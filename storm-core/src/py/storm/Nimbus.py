...
class Iface:
  ...
  def consumeMetric(self, name, value):
    """
    Parameters:
     - name
     - value
...
class Client(Iface):
  ...
  def consumeMetric(self, name, value):
    ...
    Parameters:
     ...
     - value
    """
    self.send_consumeMetric(name, value)
    self.recv_consumeMetric()

  def send_consumeMetric(self, name, value):
    self._oprot.writeMessageBegin('consumeMetric', TMessageType.CALL, self._seqid)
    args = consumeMetric_args()
    args.name = name
    args.value = value
    args.write(self._oprot)
    self._oprot.writeMessageEnd()
...
class Processor(Iface, TProcessor):
  ...
  def process_consumeMetric(self, seqid, iprot, oprot):
    args = consumeMetric_args()
    args.read(iprot)
    iprot.readMessageEnd()
    result = consumeMetric_result()
    try:
      self._handler.consumeMetric(args.name, args.value)
      msg_type = TMessageType.REPLY
    except (TTransport.TTransportException, KeyboardInterrupt, SystemExit):
      raise
    except Exception as ex:
...
class consumeMetric_args:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('consumeMetric_args')
    if self.name is not None:
      oprot.writeFieldBegin('name', TType.STRING, 1)
      oprot.writeString(self.name.encode('utf-8'))
      oprot.writeFieldEnd()
...
class consumeMetric_result:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('consumeMetric_result')
    oprot.writeFieldStop()
    oprot.writeStructEnd()
